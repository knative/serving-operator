...
import (
	...
	servingv1alpha1 "knative.dev/serving-operator/pkg/client/listers/serving/v1alpha1"
)

// Interface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.KnativeServing.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.KnativeServing. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.KnativeServing) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.KnativeServing.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.KnativeServing. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.KnativeServing) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.KnativeServing resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servingv1alpha1.KnativeServingLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.KnativeServings(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.KnativeServing, desired *v1alpha1.KnativeServing) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.OperatorV1alpha1().KnativeServings(desired.Namespace)
		...
		updater := r.Client.OperatorV1alpha1().KnativeServings(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.KnativeServing) (*v1alpha1.KnativeServing, error) {
	finalizerName := defaultFinalizerName

	getter := r.Lister.KnativeServings(resource.Namespace)
	...
	patcher := r.Client.OperatorV1alpha1().KnativeServings(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.KnativeServing) (*v1alpha1.KnativeServing, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.KnativeServing, reconcileEvent reconciler.Event) (*v1alpha1.KnativeServing, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
